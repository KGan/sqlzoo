...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    SELECT DISTINCT
      craig.num, craig.company, craig.name, sight.num, sight.company
    FROM
      routes sight
    JOIN
      routes b ON (sight.company = b.company AND sight.num = b.num)
    JOIN
      stops stopa ON sight.stop_id = stopa.id
    JOIN
      stops stopb ON b.stop_id = stopb.id
    JOIN (
      SELECT
      ...
      WHERE
        stopa.name = 'Craiglockhart'
      ) AS craig ON craig.name = stopa.name
    WHERE
